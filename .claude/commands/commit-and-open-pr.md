...
- Check for:
    ...
    - Missing error handling on external calls (GitHub API, DynamoDB, S3, LLM)
    - Security concerns (secrets in code, webhook HMAC bypass, unchecked DynamoDB writes)
    ...
    - Sealed interface `permits` clause is up to date whenever a new `LlmProvider`, `ValidationResult`, or `HealingResult` implementation is added
    - New packages follow the layout defined in `.claude/rules/api-design.md`
...
- Generate a commit message using Conventional Commits format:
    ...
    - Scope (optional): `analysis` | `generation` | `context` | `github` | `validation` | `healing` | `persistence` | `orchestration` | `infra` | `config`
    - Structure:
      ```
...
- Open a PR from the current feature branch to `main` with:
    ...
    - Description including:
        - What changed
        - Why it was needed
        - Any risks or notes for reviewers (e.g. DynamoDB schema changes, GitHub API rate limit impact, LLM prompt changes)
